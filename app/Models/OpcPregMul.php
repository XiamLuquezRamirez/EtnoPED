...
class OpcPregMul extends Model
{
    ...
    {
        $DesOpcPreg = DB::connection('mysql')->table('etno_ped.opc_mult_eval')
            ->leftJoin('etno_ped.resp_pregmultiple', 'resp_pregmultiple.respuesta', 'opc_mult_eval.id')
            ->leftJoin('etno_ped.preg_mult_eval', 'preg_mult_eval.id', 'opc_mult_eval.pregunta')
